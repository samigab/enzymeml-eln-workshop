...
def _(mo):
    mo.md(
        """
        # Step 0 — build a document, one entity at a time

        The workshop starts here, with nothing, and with one question: **what
        is a scientific data model actually made of?**

        The answer the graph gives, over and over, is *entities and the links
        between them*. Not a form, not a spreadsheet, not a file, but a set of
...
def _(csv, io):
    def read_table(
        ...
        is a file to open in pandas first and drop here afterwards.
        """
...
def _(mo):
    mo.md(
        ...
        **[step 1](01_export_eln.py)**, which turns any such document into a
        lab notebook entry. Nothing there knows this one exists: it reads a
        file, and where the file came from is none of its business.

        That indifference is worth a second of attention, because it is the
        claim the whole workshop rests on. You built a document, not an entry
        in a particular lab notebook. The next three steps put it into
        eLabFTW, take it back out and correct it in place, and every one of
        them works on any EnzymeML document, including the ones you already
        have.
        """
    )
    return
